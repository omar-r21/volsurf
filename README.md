...
surface you can query at any strike and maturity. Along the way it backs the forward and discount
...
|---:|---:|---:|---:|---:|
| 13 | 763.96 | 11.1% | 94 | 18 |
| 27 | 765.13 | 11.9% | 123 | 14 |
| 62 | 768.14 | 13.5% | 70 | 8 |
| 90 | 769.78 | 14.1% | 89 | 9 |
| 181 | 776.66 | 15.1% | 129 | 14 |
| 271 | 783.79 | 16.0% | 160 | 17 |
| 363 | 790.98 | 16.6% | 187 | 27 |
| 489 | 800.11 | 17.1% | 206 | 39 |

No butterfly or calendar arbitrage anywhere in the quoted range.
...
Forwards are recovered to about 1e-5 relative.
...
| `svi` | Raw SVI smile, multi-start least-squares fit with a butterfly-arbitrage penalty, and SSVI slices. |
...
chain = pd.read_csv("data/spy_20260919.csv")        # columns: T, strike, kind, bid, ask
surface = VolSurface.from_chain(chain, spot=761.69, rate=0.0405, min_price=0.05)

surface.implied_vol(K=700, T=0.5)                    # 0.1937
...
borrow cost, so neither has to be modelled. With no rate supplied, `D` and `F` both come from
regressing `C - P` on `K`. That works on synthetic chains, but on real near-the-money strikes
spaced $1 apart the slope is too poorly determined.
...
Its wings are linear in `k`, consistent with Lee's moment formula, so it extrapolates sensibly where
a polynomial in strike would blow up. The five-parameter least-squares problem has several local
minima, so the fit runs from 18 starting points and keeps the best.
...
The fit penalises `g < 0` on the quoted range plus a margin. A clean smile fits exactly as it would
unconstrained. Calendar arbitrage (total variance falling with maturity at fixed `k`) is checked on
...
preserves calendar-arbitrage-freeness between slices that are. Carry and rates are interpolated from
the per-expiry forwards and discount factors.
...
three quote-level arbitrage checks, and end-to-end recovery of a known surface from noisy quotes.
